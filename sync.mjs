...
const MANAGED = ["rules", "mcp", "skills", "claude", ".env.example", ".gitignore"];
...
function vendorCapabilities(gsdPaths) {
  for (const dir of CAPABILITY_DIRS) {
    ...
    for (const entry of fs.readdirSync(srcDir)) {
      ...
      report.vendored.push({ rel: destRel, tag });
      copyTree(path.join(srcDir, entry), destRel);
    }
  }
}
...
function exportRules() {
  ...
  writeFile("rules/coding-guidelines.md", clean);
  clean.split("\n").forEach((line, i) => {
    for (const noun of PROJECT_NOUNS) {
      if (new RegExp(`\\b${noun}`, "i").test(line)) {
        report.flagged.push(`rules/coding-guidelines.md:${i + 1} — "${noun}" — ${line.trim().slice(0, 70)}`);
        break;
      }
    }
  });
...
function exportSettings() {
  ...
  writeFile("claude/settings.json", JSON.stringify(JSON.parse(clean), null, 2) + "\n");
}
...
function exportPluginManifest() {
  ...
  writeFile("claude/plugins.json", JSON.stringify(manifest, null, 2) + "\n");
...
`;
  writeFile("claude/bootstrap.sh", body);
  if (!DRY) fs.chmodSync(path.join(DEST, "claude/bootstrap.sh"), 0o755);
}
...
function scanForSecrets() {
  ...
  const files = ["rules", "mcp", "skills", "claude"]
    .flatMap((d) => walk(path.join(DEST, d)))
    .concat([".env.example"].map((f) => path.join(DEST, f)).filter(exists));
