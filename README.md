...
| `skills/` | Reusable skills (portable folders with a `SKILL.md`). Includes **backend** conventions (`structure-a-backend-service`, `write-service-code`, `write-unit-tests`, `background-jobs-and-caching`, `import-data-from-csv`), **frontend** conventions (`structure-a-frontend-app`, `write-frontend-code`, `write-frontend-tests`, `structure-a-shared-ui-lib`, `secure-a-frontend-app`, `handle-files-frontend`), the shared `code-conventions` + `git-flow`, and rules extracted from the global guidelines (`release-safety`, `database-migrations`, `authoring-project-skills`). Language/framework-flexible. |
